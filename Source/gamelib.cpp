...
void CInteger::LoadBitmap()
{
	...
	//
	if (!isBmpLoaded) {
		int d[11]={IDB_0,IDB_1,IDB_2,IDB_3,IDB_4,IDB_5,IDB_6,IDB_7,IDB_8,IDB_9,IDB_INFINITY};
		for (int i = 0; i < 11; i++) 
			digit[i].LoadBitmap(d[i], RGB(0, 64, 0));
...
void CInteger::ShowBitmap()
{
	...
	if (n >= 0 && n!=INT_MAX) {
		MSB = n;
		nx = x+digit[0].Width()*(NUMDIGITS-1)*0.3;

		for (int i = 0; i < NUMDIGITS; i++) {
			int d = MSB % 10;
			MSB /= 10;
			digit[d].SetTopLeft(nx, y);
			digit[d].ShowBitmap(0.3);
			nx -= digit[d].Width()*0.3;
		}

	} 
	else {
		digit[10].SetTopLeft(x, y);
		digit[10].ShowBitmap(0.27);
	}
