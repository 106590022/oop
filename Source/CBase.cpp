...
namespace game_framework {
	int CBase::x = 450;
	int CBase::y = 300;
	CBase::CBase() :hp(new CHealthPoints(200, 10, RGB(0, 255, 127), RGB(255, 255, 255), RGB(0, 255, 127))), blood(200),time(new CDifferentTime()),distance(0),direction(0)
	{
		dx = x + 80;
		dy = y - 50;
	}
	...
	void CBase::OnShow(CGameMap& map)
	{
		...
		base.ShowBitmap();
		hp->ShowHP(map.ScreenX(x), map.ScreenY(y - 80), (int)((blood / 200.0) * 100));
		diamond.OnShow();
	}
	void CBase::OnMove()
	{
